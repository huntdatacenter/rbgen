...
public:
	...
	// Restrict this parser to a set of variants specified by the given query
	...
	// the bgen parse_genotype_data API as documented on the wiki.
	...
	// This method reads and uncompresses genotype probability data, and unpacks
	...
	// This method ignores (and skips over) genotype probability data for the current variant.
...
/* BgenParser implementation */

BgenParser::BgenParser( std::string const& filename ):
	m_filename( filename ),
	m_variant_i(0),
	m_state( e_NotOpen )
{
...
void BgenParser::set_query( IndexQuery::UniquePtr query ) {
	check_metadata( m_file_metadata, query->file_metadata() ) ;
	m_index_query = query ;
...
BgenParser::FileMetadata const& BgenParser::file_metadata() const {
	return m_file_metadata ;
}

genfile::bgen::Context const& BgenParser::context() const {
	return m_context ;
}

std::streampos BgenParser::current_file_position() const {
	return m_file_position ;
}

uint32_t BgenParser::number_of_variants() const {
...
// Data is returned in the fields of the supplied 'pack' object, which
// is defined in bgen.hpp.
void BgenParser::read_and_unpack_v12_genotype_data(
	genfile::bgen::v12::GenotypeDataBlock* pack
) {
...
void BgenParser::ignore_genotype_data() {
	assert( m_state == e_ReadyForProbs ) ;
...
// Open the bgen file, read header data and gather metadata.
void BgenParser::setup( std::string const& filename ) {
...
// Utility function to read and uncompress variant genotype probability data
// without further processing.
std::vector< byte_t > const& BgenParser::read_and_uncompress_genotype_data() {
	assert( m_state == e_ReadyForProbs ) ;
...
private:
	...
	db::Connection::UniquePtr create_bgen_index_unsafe( std::string const& bgen_filename, std::string const& index_filename ) {
		...
		BgenParser bgenView( bgen_filename ) ;
	...
	void process_selection_list( BgenParser& bgenView ) const {
